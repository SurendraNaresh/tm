...
class Logger {
  ...
  Future<void> writeLog() async {
    ...
    if (upTime != null) {
      ...
      String logData = dataStrings.join(",");
      List<String> existingLogs = prefs.getStringList(logFile) ?? [];
      existingLogs.add(logData);
      await prefs.setStringList(logFile, existingLogs);
    } else {
      print('Check Log-Creation Issues in $logFile');
    }
  }
}
